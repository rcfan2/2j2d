...
// =======================================wxpusher推送通知设置区域===========================================
//此处填您wxpusher的信息(推送appToken，例如：https://push.hellyw.com/XXXXXXXX)
//注：此处设置github action用户填写到Settings-Secrets里面（Name输入WX_PUSHER_KEY）
let WX_PUSHER_KEY = '';
let WX_PUSHER_TOPIC_IDS = []
...
if (process.env.WX_PUSHER_KEY) {
    WX_PUSHER_KEY = process.env.WX_PUSHER_KEY
}
if (process.env.WX_PUSHER_TOPIC_IDS) {
    WX_PUSHER_TOPIC_IDS = process.env.WX_PUSHER_TOPIC_IDS.split('&')
}
...
function wxpusherNotify(text, summary) {
    let data = {
        appToken: WX_PUSHER_KEY,
        content: text,
        summary: summary,
        contentType: 1,
        topicIds: WX_PUSHER_TOPIC_IDS
    }
    return new Promise(resolve => {
        if (WX_PUSHER_KEY) {
            ...
            console.log('\n您未提供wxpusher的app token，取消微信推送消息通知\n');
            resolve()
        }
    })
}
