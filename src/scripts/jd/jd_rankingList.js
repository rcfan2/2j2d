    ...
    setTimeout(() => {
      ...
      $.post(url, async(err, resp, data) => {
        try {
          // console.log(data)
          data = JSON.parse(data)
          const now = $.time('yyyy-MM-dd')
          for (const i in data.result.signTask.taskItemInfo.signList) {
          ...
          for (const i in data.result.taskList) {
...
function msgShow() {
  let message = ''
  const url = { 'open-url': `openjd://virtual?params=%7B%20%22category%22:%20%22jump%22,%20%22des%22:%20%22m%22,%20%22url%22:%20%22https://h5.m.jd.com/babelDiy/Zeus/3wtN2MjeQgjmxYTLB3YFcHjKiUJj/index.html%22%20%7D` }
