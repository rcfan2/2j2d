...
const $ = new Env('东东农场')
const cookiesArr = []; let cookie = ''; const jdFruitShareArr = []; let isBox = false; let notify; let newShareCodes
// 助力好友分享码(最多4个,否则后面的助力失败),原因:京东农场每人每天只有四次助力机会
// 此此内容是IOS用户下载脚本到本地使用，填写互助码的地方，同一京东账号的好友互助码请使用@符号隔开。
// 下面给出两个账号的填写示例（iOS只支持2个京东账号）
const shareCodes = [ // 这个列表填入你要助力的好友的shareCode
  // 账号一的好友shareCode,不同好友的shareCode中间用@符号隔开
]
let message = ''; let subTitle = ''; let option = {}; let isFruitFinished = false
...
async function doDailyTask() {
  ...
  if (!$.farmTask.gotBrowseTaskAdInit.f) {
    const adverts = $.farmTask.gotBrowseTaskAdInit.userBrowseTaskAds
    let browseReward = 0
    let browseSuccess = 0
    let browseFail = 0
    for (const advert of adverts) { // 开始浏览广告
...
async function predictionFruit() {
  ...
  await initForFarm()
  await taskInitForFarm()
  const waterEveryDayT = $.farmTask.totalWaterTaskInit.totalWaterTaskTimes// 今天到到目前为止，浇了多少次水
  ...
  // 预测n天后水果课可兑换功能
  const waterTotalT = ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy - $.farmInfo.farmUserPro.totalEnergy) / 10// 一共还需浇多少次水

  const waterD = Math.ceil(waterTotalT / waterEveryDayT)
...
async function doTenWaterAgain() {
  ...
  const overageEnergy = totalEnergy - retainWater
  ...
  if ($.masterHelpResult.code === '0') {
    ...
    if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length > 0) {
      ...
      $.masterHelpResult.masterHelpPeoples.map((item, index) => {
        ...
        const time = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getMinutes()
...
async function masterHelpShare() {
  ...
  const helpSuccessPeoplesKey = timeFormat() + $.farmInfo.farmUserPro.shareCode
...
async function clockInIn() {
  ...
  if ($.clockInInit.code === '0') {
    ...
    if ($.clockInInit.themes && $.clockInInit.themes.length > 0) {
      for (const item of $.clockInInit.themes) {
        if (!item.hadGot) {
    ...
    if ($.clockInInit.venderCoupons && $.clockInInit.venderCoupons.length > 0) {
      for (const item of $.clockInInit.venderCoupons) {
        if (!item.hadGot) {
...
async function doFriendsWater() {
  ...
  if (waterFriendCountKey < waterFriendMax) {
    ...
    if ($.friendList.friends && $.friendList.friends.length > 0) {
      ...
      let waterFriendsCount = 0; let cardInfoStr = ''
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    const jdFruitShareCodes = require('./jdFruitShareCodes.js')
