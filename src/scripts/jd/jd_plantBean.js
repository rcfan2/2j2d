...
const cookiesArr = []; let cookie = ''; const jdPlantBeanShareArr = []; let isBox = false; let notify; let newShareCodes; let option; let message
let subTitle
// 京东接口地址
...
async function doCultureBean() {
  ...
  if ($.plantBeanIndexResult.code === '0') {
    ...
    if (plantBeanRound.roundState === '2') {
      // 收取营养液
      console.log(`开始收取营养液`)
      for (const bubbleInfo of plantBeanRound.bubbleInfos) {
...
async function stealFriendWater() {
  ...
  if ($.stealFriendList.code === '0') {
    ...
    if ($.stealFriendList.data && $.stealFriendList.data.friendInfoList && $.stealFriendList.data.friendInfoList.length > 0) {
      for (const item of $.stealFriendList.data.friendInfoList) {
        if (item.nutrCount >= 3) {
  ...
  if ($.taskList && $.taskList.length > 0) {
    for (const item of $.taskList) {
      ...
      if (item.taskType === 3) {
        ...
        const goodShopListARR = []; const moreShopListARR = []; let shopList = []
        const { goodShopList, moreShopList } = data
        for (const i of goodShopList) {
          if (i.taskState === '2') {
            goodShopListARR.push(i)
          }
        }
        for (const j of moreShopList) {
          if (j.taskState === '2') {
      ...
      if (item.taskType === 5) {
        ...
        const productListARR = []; const productList = []
      ...
      if (item.taskType === 10) {
        ...
        const goodChannelListARR = []; const normalChannelListARR = []; let channelList = []
        const { goodChannelList, normalChannelList } = data
        for (const i of goodChannelList) {
          if (i.taskState === '2') {
            goodChannelListARR.push(i)
          }
        }
        for (const j of normalChannelList) {
          if (j.taskState === '2') {
...
async function doHelp() {
  for (const plantUuid of newShareCodes) {
